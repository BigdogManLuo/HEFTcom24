...
def mergeToDataset(modelling_table,target_type):
    ...
    Dataset_full=pd.read_csv(f"data/dataset/full/dwd/{target_type.capitalize()}Dataset.csv")
    Dataset_full=pd.concat([Dataset_full,modelling_table],axis=0)
    Dataset_full.to_csv(f"data/dataset/full/dwd/{target_type.capitalize()}Dataset.csv",index=False)
    
    Dataset_train=pd.read_csv(f"data/dataset/train/dwd/{target_type.capitalize()}Dataset.csv")
    Dataset_train=pd.concat([Dataset_train,modelling_table],axis=0)
    Dataset_train.to_csv(f"data/dataset/train/dwd/{target_type.capitalize()}Dataset.csv",index=False)
