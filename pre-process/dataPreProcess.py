...
if __name__ == "__main__":
    ...
    '''
    columns_wind_features_dwd,columns_solar_features_dwd=getFeaturesName("dwd")
    columns_wind_features_gfs,columns_solar_features_gfs=getFeaturesName("gfs")

    columns_wind_features=columns_wind_features_dwd+columns_wind_features_gfs
    columns_solar_features=columns_solar_features_dwd+columns_solar_features_gfs

    
    #--------------------------------- For Case I----------------------------------

    #Generate Wind and Solar Train Dataset
    IntegratedDataset_train=pd.read_csv("../data/dataset/train/IntegratedDataset.csv")
    WindDataset=IntegratedDataset_train[columns_wind_features+["Wind_MWh_credit"]]
    SolarDataset=IntegratedDataset_train[columns_solar_features+["Solar_MWh_credit"]]
    WindDataset.to_csv("../data/dataset/train/allin1/WindDataset.csv",index=False)
    SolarDataset.to_csv("../data/dataset/train/allin1/SolarDataset.csv",index=False)

    #Generate Wind and Solar Test Dataset
    IntegratedDataset_test=pd.read_csv("../data/dataset/test/IntegratedDataset.csv")
    WindDataset=IntegratedDataset_test[columns_wind_features+["Wind_MWh_credit"]]
    SolarDataset=IntegratedDataset_test[columns_solar_features+["Solar_MWh_credit"]]
    WindDataset.to_csv("../data/dataset/test/allin1/WindDataset.csv",index=False)
    SolarDataset.to_csv("../data/dataset/test/allin1/SolarDataset.csv",index=False)
    

    #--------------------------------For Case II----------------------------------
    
    #Generate Wind and Solar Train Dataset
    IntegratedDataset_full_dwd=pd.read_csv("../data/dataset/full/dwd/IntegratedDataset.csv")
    IntegratedDataset_full_gfs=pd.read_csv("../data/dataset/full/gfs/IntegratedDataset.csv")
    IntegratedDataset_full=IntegratedDataset_full_dwd.merge(IntegratedDataset_full_gfs,how="inner",on=["ref_datetime","valid_datetime"])
    IntegratedDataset_full.rename(columns={"Wind_MWh_credit_x":"Wind_MWh_credit","Solar_MWh_credit_x":"Solar_MWh_credit","total_generation_MWh_x":"total_generation_MWh","hours_x":"hours","DA_Price_x":"DA_Price","SS_Price_x":"SS_Price"},inplace=True)

    WindDataset=IntegratedDataset_full[columns_wind_features+["Wind_MWh_credit"]]
    SolarDataset=IntegratedDataset_full[columns_solar_features+["Solar_MWh_credit"]]
    WindDataset.to_csv("../data/dataset/full/allin1/WindDataset.csv",index=False)
    SolarDataset.to_csv("../data/dataset/full/allin1/SolarDataset.csv",index=False)

    #Generate Wind and Solar Test Dataset
    IntegratedDataset_latest=pd.read_csv("../data/dataset/latest/IntegratedDataset.csv")
    WindDataset=IntegratedDataset_latest[columns_wind_features+["Wind_MWh_credit"]]
    SolarDataset=IntegratedDataset_latest[columns_solar_features+["Solar_MWh_credit"]]
    WindDataset.to_csv("../data/dataset/latest/allin1/WindDataset.csv",index=False)
    SolarDataset.to_csv("../data/dataset/latest/allin1/SolarDataset.csv",index=False)
    '''