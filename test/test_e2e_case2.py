...
def train():
    ...
    model = MLPModel(input_size=features_train.shape[1], hidden_layer_size=8, output_size=1,num_layers=2).to(device)
    ...
    # Save model
    if not os.path.exists("../models/Prices/latest"):
        os.makedirs("../models/Prices/latest")
    torch.save(model.state_dict(), "../models/Prices/latest/e2e.pth")


# Preparing dataset
IntegratedDataset, modelling_table=generateModellingTable(caseNumber=2)
features_train,features_test,labels_train,labels_test,train_data,test_data,scaler_labels=trainTestSplit(modelling_table,caseNumber=2,is_E2E=True)

# Training
if not os.path.exists("../models/Prices/latest/e2e.pth"):
    train()

# Testing
model = MLPModel(input_size=features_train.shape[1], hidden_layer_size=8, output_size=1,num_layers=2).to(device)
model.load_state_dict(torch.load("../models/Prices/latest/e2e.pth"))
model.eval()
with torch.no_grad():
...
if not os.path.exists("../data/revenues/case2"):
    os.makedirs("../data/revenues/case2")
np.save("../data/revenues/case2/Revenue_e2e.npy", Revenue)
np.save("../data/revenues/case2/pd_pred_e2e.npy", pd_pred)