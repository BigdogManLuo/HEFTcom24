...
def train():
    ...
    torch.save(model.state_dict(), "../models/Prices/latest/acc_focus.pth")

# Preparing dataset
IntegratedDataset, modelling_table=generateModellingTable(caseNumber=2)
features_train,features_test,labels_train,labels_test,train_data,test_data,scaler_labels=trainTestSplit(modelling_table,caseNumber=2,is_E2E=False)

# Training
if not os.path.exists("../models/Prices/latest/acc_focus.pth"):
    train()

# Testing
model = MLPModel(input_size=features_train.shape[1], hidden_layer_size=16, output_size=1,num_layers=3).to(device)
model.load_state_dict(torch.load("../models/Prices/latest/acc_focus.pth"))
model.eval()
with torch.no_grad():
...
np.save("../data/revenues/case2/Revenue_acc_oriented.npy", Revenue)
np.save("../data/revenues/case2/pd_pred_acc_oriented.npy", outputs)
