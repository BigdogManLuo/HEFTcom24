...
if full:
    ...
    #保存模型
    for quantile in range(10,100,10):
        LGBM_models[f"q{quantile}"].save_model(f"models/LGBM/full/solar_q{quantile}.txt")
    ...
    #保存模型
    for quantile in range(10,100,10):
        LGBM_models[f"q{quantile}"].save_model(f"models/LGBM/partial/solar_q{quantile}.txt")
...
LGBM_models={}
for quantile in range(10,100,10):
    bst = lgb.Booster(model_file=f"models/LGBM/partial/solar_q{quantile}.txt")
    LGBM_models[f"q{quantile}"]=bst

#加载数据集
Labels_test=np.array(SolarDataset.iloc[:,-1])[int(0.8*len(labels)):]
...
for quantile in range(10,100,10):
    ...
    #反归一化
    y_hat=y_hat*Dataset_stats["Std"]["labels"]["solar"]+Dataset_stats["Mean"]["labels"]["solar"]

    #计算损失
    loss=pinball(y=Labels_test,y_hat=y_hat,alpha=quantile/100)

    #保存
    Solar_Generation_forecast[f"q{quantile}"]=y_hat
...
#绘制预测值和真实值的散点图
plt.scatter(Labels_test,Solar_Generation_forecast["q50"],color="blue",s=10,alpha=0.3)
