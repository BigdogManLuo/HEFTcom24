...
for quantile in range(10,100,10):
    ...
    model_wind=WindMLP(input_size=features.shape[1],
...
def train(quantile,full=False):
    ...
    model=Models_wind[f"q{quantile}"]
    ...
    for epoch in range(num_epochs):
        ...
        #将loss写入文件
        if not os.path.exists(f"training_log/wind_q{quantile}"):
            os.makedirs(f"training_log/wind_q{quantile}")

        with open(f"training_log/wind_q{quantile}/train_loss.txt", "a") as f:
            f.write(f"{train_loss_tmp}\n")
        with open(f"training_log/wind_q{quantile}/val_loss.txt", "a") as f:
            f.write(f"{val_loss_tmp}\n")
        
    #保存模型
    if full:
        torch.save(model.state_dict(), f"models/NN/full/wind_q{quantile}.pt")
    else:
        torch.save(model.state_dict(), f"models/NN/partial/wind_q{quantile}.pt")
...
def showLoss():
    ...
    for quantile in range(10,100,10):
        train_loss[quantile]=[]
        val_loss[quantile]=[]
        with open(f"training_log/wind_q{quantile}/train_loss.txt", "r") as f:
            for line in f.readlines():
                train_loss[quantile].append(float(line))
        with open(f"training_log/wind_q{quantile}/val_loss.txt", "r") as f:
            for line in f.readlines():
...
if __name__ == "__main__":
    ...
    #清空路径下的所有文件
    for quantile in range(10,100,10):
        for root, dirs, files in os.walk(f"training_log/wind_q{quantile}"):
            for name in files:
